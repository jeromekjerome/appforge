...
export function createApp({
  ...
  if (!anthropic) throw new Error('anthropic dependency is required');
  ...
  function requireAdmin(req, res, next) {
    ...
    if (!secret || secret !== env.ADMIN_SECRET) return res.status(401).json({ error: 'Unauthorized' });
    next();
  }
...
Be specific and actionable throughout. No vague requirements — this is a build spec, not a wishlist.`,
  ...
  app.post('/api/leads', async (req, res) => {
    ...
    try {
      ...
      `;
      notifyAdmin({ leadId: lead.id, name, email, spec: specMarkdown, env, mailer }).catch(console.error);
...
export async function notifyAdmin({ leadId, name, email, spec, env = process.env, mailer = nodemailer }) {
  ...
  await transporter.sendMail({
    ...
    text: `New build request #${leadId} from ${name || 'anonymous'} (${email})\n\nSpec preview:\n${spec.slice(0, 500)}...\n\nView in admin: ${env.APP_URL}/admin`,
  });
}
